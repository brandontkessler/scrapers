...
class PlayerStatsSpiderSpider(scrapy.Spider):
    ...
    def team_page(self, response):
        ...
        for game in all_game_rows:
            ...
            request = scrapy.Request(url=stats_url,
                                     ...
                                     meta={
                                        'team_index': response.meta['team_index'],
                                        'team':team,
                                        'date':date_fin,
                                        'home':home,
                                        'opponent':opponent
    ...
    def stats_page(self, response):
        ...
        # FOR THE TRAIL BLAZERS USE "TRAIL" INSTEAD OF THE TEAM VAR
        if response.meta['team_index'] == 29:
            test_for_first = "TRAIL"
        else:
            test_for_first = response.meta['team']

        # True if first in the two tables
        first_in_table = teams[0].split(" ")[1] == test_for_first
